    ...
    switch (token_symbol) {
      ...
      case "WETH": {
        ...
        if (env.mainnet.name == "ethereum") {
          ...
          if (bal.lt(amount)) {
            await mintNative(recipient, amount.mul(2));
          }
...
async function nextOfferId(base, quote, reader) {
  const [, local] = await reader.config(base, quote);
...
async function logLenderStatus(contract, lenderName, tokens, account) {
  async function getAaveBorrowBalance(symbol) {
    ...
    const vbalance = await variableDebt.balanceOf(account);
  ...
  if (lenderName == "compound") {
    baseUnit = "USD";
    [, borrowPower] = await compound.getAccountLiquidity(account);
  }
  if (lenderName == "aave") {
    baseUnit = "ETH";
    [, , borrowPower, , ,] = await pool.getUserAccountData(account);
  }
...
async function newOffer(
  ...
  const offerId = await nextOfferId(base.address, quote.address, reader);
  ...
  const offerTx = await contract.newOffer(
    ...
    wants,
    gives,
    await contract.OFR_GASREQ(), // use offer gasreq
  ...
  const book = await reader.offerList(base.address, quote.address, 0, 2);
...
async function expectAmountOnLender(account, lenderName, expectations) {
  for (const [
    ...
    switch (lenderName) {
      case "compound":
        balance = await overlying.callStatic.balanceOfUnderlying(account);
        borrow = await overlying.callStatic.borrowBalanceCurrent(account);
        break;
      case "aave":
...
async function getAccounts() {
  const [deployerSigner, makerSigner, takerSigner] = await ethers.getSigners();
  return {
    deployer: {
      signer: deployerSigner,
      address: await deployerSigner.getAddress(),
    },
    maker: { signer: makerSigner, address: await makerSigner.getAddress() },
    taker: { signer: takerSigner, address: await takerSigner.getAddress() },
  };
}

function listenMgv(mgv) {
  console.log(chalk.yellow("[LISTENING TO MANGROVE EVENTS..]"));
  const filter_posthook = mgv.filters.PosthookFail();
  mgv.once(filter_posthook, (out_tkn, in_tkn, offerId, event) => {
    console.log(chalk.red(`Posthook Reverted [Offer ${offerId}]`));
    stopListeners([mgv]);
  });
  ...
  mgv.on(
    ...
    (out_tkn, in_tkn, offerId, taker_id, takerWants, takerGives, event) => {
      console.log(
        chalk.green(`Trade success [Offer ${offerId}]`),
        `Taker ${taker_id} gave ${takerGives.toString()} and received ${takerWants.toString()}`
      );
    }
  );
  ...
  mgv.once(
    ...
    (
      ...
      console.log(
        chalk.red(`Trade Fail [Offer ${offerId}]`),
        ethers.utils.parseBytes32String(mgvData)
      );
      assert(false, ethers.utils.parseBytes32String(mgvData));
      stopListeners([mgv]);
    }
  );
}

function listenERC20(erc, ercDec) {
  console.log(chalk.yellow("[LISTENING TO ERC20..]"));
  const filter_withdraw = erc.filters.Withdrawal();
  erc.on(filter_withdraw, (taker, amount, event) => {
    console.log(
      chalk.green(
        `Account ${taker} withdrew ${ethers.utils.formatUnits(amount, ercDec)}]`
      )
    );
  });
  const filter_approval = erc.filters.Approval();
  erc.on(filter_approval, (src, trg, amount, event) => {
    console.log(
      chalk.green(
        `Account ${src}] approved ${trg} for ${ethers.utils.formatUnits(
          amount,
          ercDec
        )}`
      )
...
function installListener(logic, event) {
  return logic.filters[event];
}

function listenOfferLogic(logic, events) {
  for (const event of events) {
    logic.on(installListener(logic, event)(), (outTkn, inTkn, offerId) => {
      console.log(event, outTkn, inTkn, offerId);
    });
  }
}
...
function stopListeners(contracts) {
  for (let contract of contracts) {
    ...
    console.log(chalk.yellow("[TERMINATING LISTENER]"));
  }
}
...
exports.listenERC20 = listenERC20;
...
exports.listenOfferLogic = listenOfferLogic;
